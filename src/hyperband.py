...
def main_return(hparams, channel, layer, input_size, numit):
    # Get inputs
    ...
    if hparams.decoder_type == 'original': 
        ...
        net_fn =  partial(fixed_decodernw, num_output_channels=x_real.shape[-1], num_channels_up=num_channels, 
                image_mode=hparams.image_mode, upsample_mode=hparams.upsample_mode, filter_size=hparams.filter_size, factor=hparams.upsample_factor, input_size=input_size)
    ...
    print ('img_name:{}, mask_info:{}, decoder_type:{}, filter_size:{}, upsample_mode:{}, #channels:{} #layers:{} up_factor:{} input_size:{} WITH FINAL PSNR AS {}'.format(hparams.img_name, mask_info, 
        hparams.decoder_type, hparams.filter_size, hparams.upsample_mode, channel, layer, factor_record, input_size, psnr))
...
def get_random_hyperparameter_configuration(success_halving=False):
    ...
    if not success_halving:
        ...
        return configurations#[(32,3,64), ...]
    
    
def run_then_return_val_loss(hparams, num_iters, hyperparameters):
    psnr = main_return(hparams, hyperparameters[0], hyperparameters[1], hyperparameters[2], num_iters)
    return psnr 

def successive_halving(hparams, eta=2, max_iter=8192):   
    ...
    r = max_iter*eta**(-(s-1)) # initial number of iterations to run configurations for
    
    for i in range(s):
        n_i = n*eta**(-i)
        r_i = r*eta**(i)
        val_losses = [ run_then_return_val_loss(hparams, num_iters=r_i, hyperparameters=t) for t in T ]
        ...
        with open(hparams.pickle_file_path, 'wb') as f:
            pickle.dump(T, f)
        print('#config is {}, #iters is {}, loss criteria is {}'.format(n_i, r_i, val_losses[losses_sort_idx[int(n_i/eta)-1]]))
...
def hyperband(hparams, eta=2, max_iter=8192):
 ...
 if __name__ == '__main__':
    ...
    print('Best Config: channel is {}, layer is {}, input_size is {}'.format(out_conig[0][0], out_conig[0][1], out_conig[0][2]))