...
def fixed_decodernw(inputs,
              ...
              bn_affine=True,
              upsample_first=True,
              mtx_1D = np.array([[1],[3],[3],[1]])*1/16.,
              mtx_2D = np.array( [[1,3,3,1] , [3,9,9,3], [3,9,9,3], [1,3,3,1] ] )*1/16.
             ):
    """Deep Decoder.
       Takes as inputs a 4D Tensor (batch, width, height, channels)""" #which is the fixed noise with channels = 1
    ## Configure
...
def fit(net,
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/%s' % device):  
            ...
            z = tf.Variable(filter_cons, name='z', trainable=False)
...
def main(hparams):
    ...
    if hparams.decoder_type == 'original': 
        net_fn =  partial(decodernw, num_output_channels=x_real.shape[-1], num_channels_up=num_channels, image_mode=hparams.image_mode, upsample_mode=hparams.upsample_mode, upsample_first=False)
    elif hparams.decoder_type == 'fixed_deconv':  
        net_fn =  partial(fixed_decodernw, num_output_channels=x_real.shape[-1], num_channels_up=num_channels, image_mode=hparams.image_mode, upsample_mode=hparams.upsample_mode, upsample_first=False)
    elif hparams.decoder_type == 'deconv':  
        net_fn =  partial(deconv_decoder_real, num_output_channels=x_real.shape[-1], num_channels_up=num_channels, image_mode=hparams.image_mode, upsample_mode=hparams.upsample_mode, upsample_first=False)
