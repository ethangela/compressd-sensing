...
def _upsample(x, upsample_mode): ####################################
    if upsample_mode == '1D':
        w = tf.shape(x)[1]
        h = tf.shape(x)[2]
        new_shape = tf.stack([w * 2, h], axis=0)
        try:
...
def plot_imgs(real_img, out_img):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15,15))
    
    if real_img.shape[-1] == 3:
        ax1.imshow(real_img)
    else:
        ax1.imshow(real_img[..., 0], cmap='gray')
    ...
    ax2.set_title( "Deep-Decoder representation")
    ax2.axis('off')
    plt.show()
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--num_measurements', type=int, default=200, help='number of gaussian measurements')
