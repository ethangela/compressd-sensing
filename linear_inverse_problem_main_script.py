...
def _upsample(i, x, upsample_mode, image_mode, factor=None, layer=None, input_size=128, signal_size=(128,128)): 
    if upsample_mode == 'none':
        ...
        if factor:
            ...
            elif image_mode == '2D' or image_mode == '3D':
        ...
        if layer:
            ...
            w_new = tf.cast(round(input_size * pow(signal_size[0]/input_size, i/layer)), tf.int32) ##3D
            h_new = tf.cast(round(input_size * pow(signal_size[1]/input_size, i/layer)), tf.int32) ##3D
            if image_mode == '1D':
                new_shape = tf.stack([w_new, h], axis=0)
            elif image_mode == '2D' or image_mode == '3D':
                new_shape = tf.stack([w_new, h_new], axis=0)
...
def decodernw(inputs,
    ...
    for i, (num_channels, kernel_size) in enumerate(zip(channels_times_layers, filter_size_up)): 
        ...
        # Upsample 
        net = _upsample(i+1, net, upsample_mode, image_mode, factor, len(channels_times_layers), input_size, signal_size)
...
def fit(net,
    ...
    """Fit a model.
        ...
        img_shape: original real image shape, a 4D tensor, e.g. [1,128,128,3] 
    ...
    with tf.Graph().as_default():
        ...
        def circulant_tf(signal_vector, random_vector, selection_mask):  
...
def main(hparams):
    ...
    channels_times_layers = [hparams.k]*(hparams.num_layers-1)   
    ...
    if hparams.decoder_type == 'original': 
        net_fn =  partial(decodernw, num_output_channels=x_real.shape[-1], channels_times_layers=channels_times_layers, image_mode=hparams.image_mode, 
                upsample_mode=hparams.upsample_mode, factor=hparams.upsample_factor, input_size=hparams.input_size, act_fun=act_function, signal_size=(x_real.shape[1],x_real.shape[2]))
        ...
        net_fn =  partial(fixed_decodernw, num_output_channels=x_real.shape[-1], channels_times_layers=channels_times_layers, image_mode=hparams.image_mode, 
                upsample_mode=hparams.upsample_mode, filter_size=hparams.filter_size, factor=hparams.upsample_factor, input_size=hparams.input_size, act_fun=act_function, signal_size=(x_real.shape[1],x_real.shape[2]))
    ...
    if hparams.upsample_factor:
        ...
        factor_record1 = round(1 * pow(x_real.shape[1]/hparams.input_size, 1/(hparams.num_layers-1)), 4) ##3D
        factor_record2 = round(1 * pow(x_real.shape[2]/hparams.input_size, 1/(hparams.num_layers-1)), 4) ##3D
        
    print ('Final representation PSNR for img_name:{}, model_type:{}, type_mea:{}, num_mea:{}, mask:{}, decoder:{}, filter_size:{}, upsample_mode:{}, #channels:{} #layers:{} up_factor1:{} up_factor2:{}: input_size:{} act_func:{} step_size:{} noise:{} is {}'.format(hparams.img_name, 
            hparams.model_type, type_mea_info, num_mea_info, mask_info, 
            hparams.decoder_type, hparams.filter_size, hparams.upsample_mode, hparams.k, hparams.num_layers, factor_record1, factor_record2, hparams.input_size, hparams.activation, hparams.lr, noise_level_info, psnr))
    ...
    if hparams.hyperband_mode == 0:
        ...
        if not os.path.exists(pickle_file_path):
            d = {'img_name':[hparams.img_name], 'model_type':[hparams.model_type], 'type_mea':[type_mea_info], 'num_mea':[num_mea_info], 'mask_info':[mask_info], 
                ...
                'channels':[hparams.k], 'layers':[hparams.num_layers], 'up_factor1':[factor_record1], 'up_factor2':[factor_record2], 'input_size':[hparams.input_size], 'act_func':[hparams.activation], 'step_size':[hparams.lr], 'noise':[noise_level_info], 'psnr':[psnr]}
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--input_size', type=int, default=16, help='input_size') 
    ...
    PARSER.add_argument('--lr', type=float, default=0.01, help='reg_noise_std')
